...
def process(band="L", ID_Pass=0, x_hist='entropy'):
    ...
    ds_l = combine_netcdf(prmasr_nc[band]["18"][ID_Pass], prmasr_nc[band]["19"][ID_Pass])
    # ds_s = combine_netcdf(prmasr_nc["S"]["18"][ID_Pass], prmasr_nc["S"]["19"][ID_Pass])
    ...
    for item in pol_key:
        print(item)
        if x_hist != item:
            file_name = prmasr_nc[band]["18"][ID_Pass].name[:-3] + "_" + prmasr_nc[band]["19"][ID_Pass].name[:-3]
            pol_key_name = item
